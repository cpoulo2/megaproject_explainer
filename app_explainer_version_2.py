...
Remember, the money missed out on is <b><font style="background-color:#FFC107;">cumulative</font></b>, which means it <b><font style="background-color:#FFC107;">adds up over time</font></b>. In the first year your district would miss out on \${df_project["Potential Tax Revenue (Inflated)"].values[0]:,.0f}. In the second year, your district would still be missing the \${df_project["Potential Tax Revenue (Inflated)"].values[0]:,.0f} from the first year, plus more money missed out on in the second year of the tax break: \${df_project["Potential Tax Revenue (Inflated)"].values[1]:,.0f}. This accumulates over the {tax_break_term}-year tax break. The chart shows the <b>cumulative potential revenue</b> and the <b>cumulative special payment</b> over the tax break term.   
</p>""", unsafe_allow_html=True)
